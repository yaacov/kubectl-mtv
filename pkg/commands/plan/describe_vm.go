...
import (
	...
	"github.com/yaacov/kubectl-mtv/pkg/commands/plan/status"
...
func describeVMOnce(configFlags *genericclioptions.ConfigFlags, name, namespace, vmName string) error {
	...
	if started != "" {
		fmt.Printf("%s %s\n", output.Bold("Started:"), formatTime(started))
	}
	if completed != "" {
		fmt.Printf("%s %s\n", output.Bold("Completed:"), formatTime(completed))
	}
	...
	if exists {
		...
		for _, p := range pipeline {
			...
			if phaseCompleted != "" {
				fmt.Printf("%s %s\n", output.Bold("Completed:"), formatTime(phaseCompleted))
			}
