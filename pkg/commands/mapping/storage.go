...
func parseStoragePairs(pairStr, defaultNamespace string, configFlags *genericclioptions.ConfigFlags, sourceProvider, inventoryURL string) ([]v1beta1.StoragePair, error) {
	if pairStr == "" {
		return nil, nil
	}

	var pairs []v1beta1.StoragePair
...
func createStorageMapping(configFlags *genericclioptions.ConfigFlags, name, namespace, sourceProvider, targetProvider, fromFile, storagePairs, inventoryURL string) error {
	...
	var mappingPairs []v1beta1.StoragePair
	if storagePairs != "" {
