...
class TestOpenStackProviderPatch:
    ...
    def _verify_credential_in_secret(
        ...
        if credential_field in secret_data:
            ...
            assert (
                credential_field in string_data
            ), f"Credential field '{credential_field}' not found in secret"
    ...
    def test_patch_openstack_provider_password(
        self, test_namespace, openstack_provider, provider_credentials
    ):
        ...
        patch_cmd = (
            f"patch provider {openstack_provider} --password '{creds['password']}'"
        )
        ...
        self._verify_credential_in_secret(
            test_namespace, openstack_provider, "password", creds["password"]
        )

    def test_patch_openstack_provider_username(
        self, test_namespace, openstack_provider, provider_credentials
    ):
        ...
        patch_cmd = (
            f"patch provider {openstack_provider} --username '{creds['username']}'"
        )
        ...
        self._verify_credential_in_secret(
            test_namespace, openstack_provider, "username", creds["username"]
        )

    def test_patch_openstack_provider_domain_name(
        self, test_namespace, openstack_provider, provider_credentials
    ):
        ...
        patch_cmd = f"patch provider {openstack_provider} --provider-domain-name '{creds['domain_name']}'"
        ...
        self._verify_credential_in_secret(
            test_namespace, openstack_provider, "domainName", creds["domain_name"]
        )

    def test_patch_openstack_provider_project_name(
        self, test_namespace, openstack_provider, provider_credentials
    ):
        ...
        patch_cmd = f"patch provider {openstack_provider} --provider-project-name '{creds['project_name']}'"
        ...
        self._verify_credential_in_secret(
            test_namespace, openstack_provider, "projectName", creds["project_name"]
        )

    def test_patch_openstack_provider_region_name(
        self, test_namespace, openstack_provider, provider_credentials
    ):
        ...
        patch_cmd = f"patch provider {openstack_provider} --provider-region-name '{creds['region_name']}'"
        ...
        self._verify_credential_in_secret(
            test_namespace, openstack_provider, "regionName", creds["region_name"]
        )

    def test_patch_openstack_provider_multiple_fields(
        self, test_namespace, openstack_provider, provider_credentials
    ):
        ...
        if creds.get("domain_name"):
            cmd_parts.append(f"--provider-domain-name '{creds['domain_name']}'")
        if creds.get("project_name"):
            cmd_parts.append(f"--provider-project-name '{creds['project_name']}'")
        ...
        get_result = test_namespace.run_mtv_command(
            f"get provider {openstack_provider} -o yaml"
        )
        assert get_result.returncode == 0
