...
class TestOpenShiftProviderPatch:
    ...
    def openshift_provider(self, test_namespace, provider_credentials):
        ...
        # Create OpenShift provider - use simple localhost provider for testing
        cmd_parts = [
            "create provider",
            provider_name,
            "--type openshift",
    ...
    def test_patch_openshift_provider_url(
        self, test_namespace, openshift_provider, provider_credentials
    ):
        ...
        get_result = test_namespace.run_mtv_command(
            f"get provider {openshift_provider} -o yaml"
        )
        assert get_result.returncode == 0
        assert creds["url"] in get_result.stdout

    @pytest.mark.skip(
        reason="Cannot add credentials to provider created without secrets - kubectl-mtv limitation"
    )
    def test_patch_openshift_provider_token(
        self, test_namespace, openshift_provider, provider_credentials
    ):
    ...
    def test_patch_openshift_provider_insecure_skip_tls(
    ...
    def test_patch_openshift_provider_cacert(
        self, test_namespace, openshift_provider, provider_credentials
    ):
    ...
    def test_patch_openshift_provider_multiple_fields(
        self, test_namespace, openshift_provider, provider_credentials
    ):
        ...
        assert (
            "insecure: true" in get_result.stdout
            or "skipTLS: true" in get_result.stdout
        )
